...
import { exportTasksToJSON, importTasksFromJSON, type ValidationResult } from '@/utils';
...
function ActionsDropdown() {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      console.error(error);
      message.error(error instanceof Error ? error.message : t('importExport.importError'));
    } finally {
  ...
  function showValidationReport(result: ValidationResult) {
    ...
    const errorList = Array.from(errorsByTask.entries())
      .map(([taskIndex, errors]) => {
        const taskTitle =
          errors[0].taskTitle || t('importExport.validationTaskFallback', { index: taskIndex });
  ...
  return (
    <>
      ...
      <SettingsModal open={isSettingsOpen} onClose={onCloseSettings} data-tour="settings-button" />
