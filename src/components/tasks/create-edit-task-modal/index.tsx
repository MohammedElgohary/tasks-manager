import { cloneElement, useId, useState, useEffect, memo, lazy, Suspense } from 'react';
...
import type { CreateEditTaskButtonProps, CreateEditTaskModalProps, FormValues } from './interface';
import dayjs from 'dayjs';
import { PrioritySelect, StatusSelect } from '@/components';
import { useTranslation } from 'react-i18next';
import { useTasksStore } from '@/stores';

const CreateEditTaskModal = lazy(() => Promise.resolve({ default: CreateEditTaskModalComponent }));

function CreateEditTaskButton({ trigger, initialValues }: CreateEditTaskButtonProps) {
...
function CreateEditTaskModalComponent({
  ...
  async function onFinish(values: FormValues): Promise<void> {
    ...
    try {
      ...
      message.success(isEditing ? t('taskModal.updateSuccess') : t('taskModal.createSuccess'));
    } catch (error) {
      console.error(error);
      message.error(isEditing ? t('taskModal.updateError') : t('taskModal.createError'));
    }
  }
  ...
  return (
    <Modal
      open={open}
      onCancel={onClose}
      title={initialValues ? t('taskModal.editTitle') : t('taskModal.createTitle')}
      centered
      footer={
        <Flex justify="end" gap="small">
          ...
          <Button type="primary" htmlType="submit" form={FORM_ID}>
            {initialValues ? t('taskModal.updateButton') : t('taskModal.createButton')}
          </Button>
        </Flex>
      }
    >
      ...
      >
        <Flex vertical>
          ...
          >
            <Input placeholder={t('taskModal.titlePlaceholder')} maxLength={100} showCount />
          </Form.Item>
          ...
          <Flex gap="small">
            <Form.Item name="status" label={t('task.status')} className="flex-1">
              <StatusSelect placeholder={t('taskModal.statusPlaceholder')} />
            </Form.Item>

            <Form.Item name="priority" label={t('task.priority')} className="flex-1">
              <PrioritySelect placeholder={t('taskModal.priorityPlaceholder')} />
            </Form.Item>
          </Flex>
          ...
          <Form.Item
            ...
            getValueFromEvent={(date: dayjs.Dayjs | null) => (date ? date.toISOString() : null)}
          >
            <DatePicker
